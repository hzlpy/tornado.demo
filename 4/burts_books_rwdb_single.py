...
class BookHandler(tornado.web.RequestHandler):
    def get(self, isbn=None):
        if isbn:
            ...
            book = self.application.db.get(sql)
            if book:
...
class BookEditHandler(tornado.web.RequestHandler):
    ...
    def post(self, isbn=None):
        ...
        if isbn:    # 更新
            sql = "UPDATE book SET title = '%s', subtitle = '%s', image = '%s', author = '%s', date_added = '%s', date_released = '%s', description = '%s' WHERE isbn = '%s'" % (title, subtitle, image, author, date_added, date_released, description, isbn)
